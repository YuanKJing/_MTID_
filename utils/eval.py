    ...
    for i_batch, sample_batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            task_onehot = torch.zeros(
                (task_class.size(0), args.class_dim))  # [bs*T, ac_dim]
            ...
            action_label_onehot = torch.zeros(
                (video_label_reshaped.size(0), args.action_dim))
            ...
            action_label_onehot = action_label_onehot.reshape(
                batch_size_current, T, -1).cuda()

            x_start = torch.zeros(
                (batch_size_current, T, args.class_dim + args.action_dim + args.observation_dim))
            x_start[:, 0, args.class_dim +
                    args.action_dim:] = global_img_tensors[:, 0, :]
            x_start[:, -1, args.class_dim +
